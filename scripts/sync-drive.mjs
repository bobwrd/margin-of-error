...
// tree. Run by .github/workflows/sync-drive.yml on a schedule (and manually).
//
// Auth: a Google service account. Put the full JSON key in the GitHub secret
...
//       *.txt                   -> content/verdict/
//       *.md (cross-post posts) -> content/articles/
//
// Exit code 0 always (so the workflow's commit step decides whether anything
// changed). Errors during auth/listing throw and fail the job.
...
// Per sub-folder routing. `dest` is the default directory; `route(name)` can
// override the destination path for individual files.
const FOLDER_ROUTES = {
  "Daily Brief": { dest: "content/daily_findings" },
  "Weekly Article": { dest: "content/articles" },
  "Personal Pieces": { dest: "content/articles" },
  "Verdict Radar": {
    dest: "content/articles",
    route(name) {
      ...
      return `content/articles/${name}`; // cross-post articles
    },
  },
};
...
async function run() {
  ...
  for (const folder of subFolders) {
    ...
    for (const file of files) {
      const relPath = routing.route
        ? routing.route(file.name)
        : `${routing.dest}/${file.name}`;
