...
export default function LedgerActionPage() {
  ...
  if (!action) return (
    <div className="py-16 text-center text-sm font-mono" style={{ color: "var(--ledger-muted)" }}>
      Action not found. <Link to="/ledger" style={{ color: "var(--ledger-accent)" }}>Back to register →</Link>
    </div>
  );

  return (
    <div className="max-w-2xl">
      <Link to="/ledger" className="text-xs font-mono opacity-60 hover:opacity-100 transition-opacity" style={{ color: "var(--ledger-muted)" }}>
        ← Register
      </Link>
