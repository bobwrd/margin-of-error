...
export default function LedgerLayout({ children }: { children: React.ReactNode }) {
  ...
  const navLinks = [
    { label: "Register", href: "/ledger" },
    { label: "Charts", href: "/ledger/charts" },
    { label: "Methodology", href: "/ledger/about" },
  ];
      ...
      >
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
          ...
          <nav className="flex items-center gap-0.5">
            {navLinks.map((link) => {
              const active =
                link.href === "/ledger" ? pathname === "/ledger" : pathname.startsWith(link.href);
              return (
                <Link
                  key={link.href}
                  to={link.href}
      ...
      <footer className="border-t mt-20" style={{ borderColor: "var(--ledger-border)" }}>
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
          <div className="flex items-center justify-between text-xs font-mono" style={{ color: "var(--ledger-muted)" }}>
            <span>THE LEDGER · Margin of Error</span>
            <Link
              to="/ledger/about"
