// Cloudflare Worker entry point — the entire /api/* surface.
//
// This is the port of the original Bun + Hono `server.ts`, adapted to the
// Workers + Static Assets model (Cloudflare folded Pages into Workers). The
// React build in dist/ is served by the static-assets layer; this Worker only
// runs for /api/* (see wrangler.toml `run_worker_first`). Every route is
// identical to the old server, so the frontend needs no changes.
//
//   - Content (articles, profile, verdict cases) is read from a build-time
//     baked JSON module (functions/generated/content.json) — Workers have no
//     filesystem at runtime.
//   - Dynamic tables (likes, contact, newsletter, verdict drafts) use D1.
//
// Bindings: D1 database `DB`, secret `VERDICT_PASSWORD`.

import { Hono } from "hono";
import baked from "./functions/generated/content.json";
...
app.get("/content/weekly", (c) =>
  c.json({
    items: loadAllArticles().filter((i) => i.category === "weekly").map(stripBody),
  })
);
app.get("/content/personal", (c) =>
  c.json({
    items: loadAllArticles().filter((i) => i.category === "personal").map(stripBody),
  })
);
app.get("/content/other", (c) =>
  c.json({
    items: loadAllArticles().filter((i) => i.category === "other").map(stripBody),
  })
);
app.get("/content/short", (c) =>
  c.json({
    items: loadAllArticles().filter((i) => i.category === "short").map(stripBody),
  })
);
// Legacy aliases
